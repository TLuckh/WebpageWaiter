...
namespace MSAALister
{
	class Program
	{
		...
		private static bool EnumWindows(IntPtr hWnd, IntPtr lParam)
		{
			...
			{
				sFile.WriteLine("Exception trying to get url: " + ex);
			}
		...
		private static void LogAccessibleObject(int depth, IntPtr hWnd, IAccessible child)
		{
			...
			sFile.WriteLine("{0}[{1}]:{2}={3}, Role:{4}, State:{5}, Pos:{6},{7}", hWnd.ToString("X"), GetClassName(hWnd), name, value, roleString, state, left, top);
		}
		...
		{
			var doc = GetFirefoxDocument(hwnd);

			return doc?.accValue[0];
		}

        protected static IAccessible GetFirefoxDocument(IntPtr hwnd)
        {
            var application = AccessibleObjectHelper.FindChild(AccessibleObjectHelper.GetAccessibleObjectFromWindow(hwnd),
		...
		{
			var ieServerHwnd = FindDescendantWindows(hwnd, "Internet Explorer_Server").FirstOrDefault();
			if (ieServerHwnd == null)
			{
				return null;
			}
